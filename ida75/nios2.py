...
class nios2_processor_t(idaapi.processor_t):
	...
	def notify_out_operand(self, ctx, op):
		...
		if optype == o_reg:
			...
			if ctx.insn.itype == self.itype_jmp and get_switch_info(ctx.insn.ea) == None:
	...
	def check_stkvar_off(self, insn):
		if insn.Op1.is_reg(self.ireg_sp) == False and insn.Op2.is_reg(self.ireg_sp) and insn.Op3.type == o_imm:
	...
	def trace_sp(self, insn):
		...
		if insn.Op1.is_reg(self.ireg_sp) and insn.Op2.is_reg(self.ireg_sp) and insn.Op3.type == o_imm:
	...
	def check_jmp_reg(self, insn):
		...
		if insn.itype == self.itype_jmp:
			...
			while prev.ea > CurFuncStart and decode_preceding_insn(prev, prev.ea) != BADADDR and CurCmdCnt < MAX_SEARCH_CMD_RANGE:
				if (prev.itype == self.itype_addi or prev.itype == self.itype_subi or prev.itype == self.itype_ori) and self.is_op_reg(prev.Op1,JumpAddrRegLow) and JumpAddrRegHigh == None:
				...
				elif prev.itype == self.itype_movhi and self.is_op_reg(prev.Op1,JumpAddrRegHigh) and JumpOffHigh == None:
	...
	def check_switch(self, insn):
		...
		if insn.itype == self.itype_jmp:
			...
			decode_insn(prev, insn.ea)
			while prev.ea > CurFuncStart and decode_preceding_insn(prev, prev.ea) != BADADDR and CurCmdCnt < MAX_SEARCH_CMD_RANGE:
				if prev.itype == self.itype_ldw and self.is_op_reg(prev.Op1, RegJump) and prev.Op2.addr == 0 and JumpOffReg == None:
					JumpOffReg = prev.Op2.reg
				elif prev.itype == self.itype_add and self.is_op_reg(prev.Op1, JumpOffReg) and JTableAddrRegLow == None:
					TempCaseReg = prev.Op2.reg
					JTableAddrRegLow = prev.Op3.reg
				elif (prev.itype == self.itype_addi or prev.itype == self.itype_subi or prev.itype == self.itype_ori) and self.is_op_reg(prev.Op1, JTableAddrRegLow) and JTableAddrRegHigh == None:
				...
				elif prev.itype == self.itype_movhi and self.is_op_reg(prev.Op1, JTableAddrRegHigh) and JTableOffHigh == None:
					JTableOffHigh = prev.Op2.value & 0xFFFFFFFF
					if SubiFlag:
						JTableAddr = (JTableOffHigh << 16) - JTableOffLow
					else:
						JTableAddr = (JTableOffHigh << 16) + JTableOffLow
				elif (prev.itype == self.itype_slli or prev.itype == self.itype_sll or prev.itype == self.itype_mul) and self.is_op_reg(prev.Op1,TempCaseReg) and CurCaseReg == None:
					CurCaseReg = prev.Op2.reg
				elif (prev.itype == self.itype_bltu and self.is_op_reg(prev.Op2, CurCaseReg)) and MaxCaseNumReg == None:
					DefJump = prev.Op3.addr
					MaxCaseNumReg = prev.Op1.reg
				elif prev.itype == self.itype_bne and prev.Op2.is_reg(0) and MaxCaseNumReg == None:
					DefJump = prev.Op3.addr
					MaxCaseNumReg = prev.Op1.reg
				elif prev.itype == self.itype_bgeu and self.is_op_reg(prev.Op2, CurCaseReg) and MaxCaseNumReg == None:
					DefJump = next_head(prev.ea, BADADDR)
					MaxCaseNumReg = prev.Op1.reg
				elif prev.itype == self.itype_movi and self.is_op_reg(prev.Op1, MaxCaseNumReg) and NumOfCases == 0:
					NumOfCases = cmd.Op2.value + 1
					NoNeedToCheck = True
				elif prev.itype == self.itype_cmpgeui and self.is_op_reg(prev.Op1, MaxCaseNumReg) and NumOfCases == 0:
				...
				elif prev.itype == self.itype_mov and ((self.is_op_reg(prev.Op1, CurCaseReg) and self.is_op_reg(prev.Op2, RegToCheck)) or (self.is_op_reg(prev.Op1, RegToCheck) and self.is_op_reg(prev.Op2, CurCaseReg))):
					NoNeedToCheck = True
			...
			if insn.Op2.type == o_reg and insn.Op3.type == o_imm:
				...
				if decode_prev_insn(prev, insn.ea) != BADADDR:
					if prev.itype == self.itype_movhi:
						if prev.Op1.type == o_reg and prev.Op1.is_reg(OffReg) and prev.Op2.type == o_imm:
